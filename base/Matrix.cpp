...
const Matrix Matrix::outerProduct(const Vector a, const Vector b){
...
const Vector Matrix::vecMultMat(const Vector& x) const{
  ...
  //std::cout << "Mat mult vec result: " << result.X() << " " << result.Y() << " " << result.Z() << std::endl;
  return result;
}
